...
Additional inserts created by Francesca Macasinag for DMIT 1508 Mock Quizzes
*/

--create database MemoriesForeverSPQuiz

-- Make sure a new database is created to avoid table conflicts
...
Phone varchar(10) not null
constraint ck_validPhone check(Phone like '[1-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]'),
Wage smallmoney not null 
constraint df_wage default 15
constraint ck_wage check(Wage >= 15),
...
Days int not null constraint ck_Days check (days>0),
...
Insert into Project(ProjectDescription, InDate, OutDate, Estimate, ProjectTypeCode, ClientID, SubTotal, GST, Total, StaffID) Values ('Jackson Dale 4th Birthday', 'October 28, 2013', 'November 5, 2013', 130, 4, 1, 120, 6, 126, 4) --id 6
