...
Mock quiz created by Francesca Macasinag for SL Sessions Fall 2018.
*/


--Instructions: 
--Part 1: create the database and fill in the constraints for the Staff table. Once done, execute the code for all the create tables

--create database MemoriesForeverQuiz

-- Make sure this database is created and uses the proper name
...
--Part 1
--Add the following constraints to the Staff table definitions.

--Wage should have a default of $15 to apply the updated minimum wage restrictions
--The database should check that the wage is greater than or equal to $15
--Phone number should be a 10 digit number and it cannot start with 0, it should not have any other special characters
--Hire date should be before or on the same date as the current date
--StaffTypeID in the Staff table is a foreign key for the StaffType table
...
Insert into ProjectType(ProjectTypeCode, ProjectTypeDescription) Values (6, 'Graduation')
...
--Memories Forever decided that they want to set themselves apart from other businesses and pay their staff more than minimum. Alter the table to delete the original check constraint, then add a new one to check that wage is greater than $18 (no need to change the default)



--Part 3

--1. Select the names of staff members (as one column) making over $30 an hour


--2. Select the staff type description with more than one staff in it
...
--5. Select the Project ID and Client Name for projects that have an out date on or after November 2012. Use a subquery.


--6. Select the project type description that have not been used for a project


--7. Select the ItemID and the model numbers for all Sennheiser items the company owns. The owner only wants to see the model numbers so do not show the brand "Sennheiser" in the results, and rename the column as "SennheiserModel#" instead. Hint: these items all start with "Sennheiser" and the maximum number of characters for Sennheiser model numbers is 10


--8. Select the staff names and the number of projects they have worked on. Show only the ones who are staff type 3 and have worked on more than one project


--9. Memories Forever made an error in recording information for the McMillan Wedding (ProjectID 2). It was actually shot in Kelowna, BC, not Tomlinson, AB and the client (ClientID 3)  is also from there. They also realized that they have to follow BC sales tax regulations, and they should have charged 7% instead of 5% tax. Update the necessary columns to reflect these changes. Hint: Do 2 separate update statements.
